...
describe('the README front matter carries what R13.9 requires', () => {
  ...
  it('names the deployed URL and the demo command in exactly one place each', () => {
    /* Two copies of a URL is two things to edit, and the second one is the one that
       ...
       ambiguity about which one it is looking at. */
    const urls = FRONT_MATTER.flatMap(httpsUrls);
    expect(
      urls.length,
      `the front matter carries ${urls.length} https URLs (${urls.join(', ')}). Exactly ` +
        `one — the deployed Ledger — or none while it is pending. Put the repository ` +
        `URL and any documentation links below line ${LINE_BUDGET}.`,
