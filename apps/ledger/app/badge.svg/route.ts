 ...
 * SVG and names its headers; `lib/snapshot.ts` is the one place the snapshot is
 * read and validated. What is left is the boundary itself, which is why Property 25
 * can assert the badge over a thousand generated snapshots without a server.
 *
 * The figure is `metrics.provenCoverage`, straight from the snapshot and never
 * recomputed. It is `null` while the graph is degraded, so the badge reads `n/a`
 * with no division performed (R9.3) — and that is the live path today, which is the
 * point: a badge that showed `0%` for a run that measured nothing would be the one
 * dishonest pixel on the page.
 */
