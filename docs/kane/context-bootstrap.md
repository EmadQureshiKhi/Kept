...
progress that does not yet lift `degraded`.
...
`apps/ledger/data/ledger.snapshot.json` was **left untouched**. It is honestly
`degraded: true` with `degradedReason: assurance-status:refused`, which is still exactly
what a build reports. Its diagnostic quotes the older *no store* message; that is now
stale prose inside an accurate verdict, and it will be regenerated by the first build that
actually clears the refusal rather than churned here to look busier.

## The store is not yet enough for `resolveSourceId` — two independent blockers

The task text says this pair is "the precondition for `resolveSourceId` finding a match at
all". The precondition is now met and the match still does not happen. Both blockers were
observed, not reasoned about, and both belong to task 15.4.
