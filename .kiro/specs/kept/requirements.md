...
- **A16 Distribution**: `kept-core` and `@corgod/kept-cli` are published to the public npm registry so the engine runs against a repository that is not this one. Both manifests are `private: true` at version `0.0.0` today, and `@corgod/kept-cli` depends on `kept-core` by the literal `0.0.0`, which resolves only inside this workspace. Publishing is therefore a manifest and packaging task, gated on a test that installs the packed tarball outside the workspace and runs the binary from there. A published package that only works in its own monorepo is the same class of defect this project exists to detect.
...
- **Published_Package**: One of the two npm packages `kept-core` and `@corgod/kept-cli` as installed from the public registry rather than resolved from the workspace.
...
9. THE repository SHALL carry one recorded end-to-end docs-trigger cycle in which a claim absent from the product is added to the Fixture_App documentation, reconciliation reports it as outstanding suite debt, a designed test is bound to it, verification of that test returns a failure the Verdict_Router routes to `docs-lie`, and a Docs_Amendment is produced without any documentation, test or source file being written.
10. THE recorded cycle SHALL commit the reconciliation stream, the verification stream, both Handoff_Files, the resulting Docs_Amendment and the Ledger_Snapshot that renders it, so the cycle is reproducible from committed bytes with Kane_CLI invoked zero times.
11. WHILE the recorded cycle is being produced, THE Fixture_App documentation SHALL be returned to its committed content, so that the repository's own eight claims are unchanged by the demonstration.

*Why 9 to 11 exist. Requirement 5's machinery is implemented and a live `maintain reconcile --plan` run is recorded, but no single continuous cycle has been captured from "documentation now claims something untrue" through to "an amendment is proposed and nothing was written". That cycle is the docs half of the closed loop and the only branch of the three that no competing approach demonstrates; leaving it as separately-recorded fragments makes it the weakest of the two triggers rather than the more novel one.*
...
10. THE KEPT_CLI SHALL invoke `maintain evolve` as `kane-cli maintain evolve <ref>` with no ask-policy flag, SHALL rely on piped stdout as the NDJSON enabler for that command, and SHALL treat the event whose `type` equals `done` as its Terminal_Event.
11. WHEN a `maintain evolve` invocation delivers a clean terminal event, THE KEPT_CLI SHALL build the Review_Card from the pair diff that invocation reports, and SHALL fall back to the failure context only when it does not.

*Correction to 2, measured against `kane-cli` 0.8.4. `maintain evolve --help` lists exactly two options — `--from-stale` and `--because` — and rejects `--mode agent` with `unknown option '--mode'`, while `maintain reconcile --help` from the same command group does list `--mode <mode>`. The asymmetry is real rather than an abridged help table, so 2's spelling can never succeed on this release and 10 states the invocation that can. 2 is left in place as the contract for a release that carries the flag; the degradation it describes is what ships today and 11 is what replaces it once the corrected argv is wired.*
...
9. THE Enrichment_Provider SHALL obtain the Coverage_Axes from `kane-cli cover gaps --json --mode agent`, which derives them from the live assurance graph, and SHALL NOT require an Evidence_Pack to carry a coverage document.
10. THE Coverage_Axes SHALL comprise a Design_Completeness percentage with its designed-acceptance-criteria ratio, and a Proven percentage with its proven-acceptance-criteria ratio, each read verbatim from the `gaps` payload.
11. THE Enrichment_Provider SHALL project one Coverage_Row per use-case carrying that use-case's identifier, title, risk band, Design_Completeness percentage and status, Proven percentage and status, stale-acceptance-criteria count, and every pending item with its stage, reason and ready command.
12. THE Ledger SHALL render the Coverage_Axes as a dual-axis ribbon on the shareable public page, showing Design_Completeness against Proven per use-case, ordered by risk band and then by identifier.
13. WHERE the `gaps` invocation does not deliver a clean terminal event, THE Promise_Graph SHALL remain degraded and THE Ledger SHALL withhold the Coverage_Axes rather than render a zero.
14. THE Coverage_Axes SHALL be recorded in the Ledger_Snapshot so that the shareable page renders them with Kane_CLI invoked zero times.
15. THE Ledger SHALL distinguish Proven_Coverage, which counts Promises this repository verified, from the Proven axis of the Coverage_Axes, which counts acceptance criteria Kane's graph records execution facts for, and SHALL label each so neither is read as the other.

*Why 9 exists, measured against `kane-cli` 0.8.4. The singular `cover` command reads its depth axis out of a sealed Evidence_Pack and refuses with `carries no coverage/usecases.yaml — the pack predates coverage or its project had no .context at seal time` at exit 2, because a pack sealed by a **replay** carries no coverage document and only an authoring run mints one. `cover gaps` answers the same two axes from the graph instead: exit 0, `status: complete`, `design_completeness` and `proven` blocks plus a per-use-case dossier, with `proven.config.source: graph_execution_facts` and `denominator: current_live_acs`. The axis the original design wanted is therefore reachable without running the interactive, credit-metered design chain — which is what R9.9 pins so the dependency is not quietly reintroduced.*
...
2. THE packages `kept-core` and `@corgod/kept-cli` SHALL contain, in their executable source, no literal path naming the Fixture_App, no literal port number of the Fixture_App, and no literal Corpus_Root.
...
1. THE packages `kept-core` and `@corgod/kept-cli` SHALL be publishable to the public npm registry, and neither published manifest SHALL carry `private` set to true.
2. THE `@corgod/kept-cli` manifest SHALL depend on `kept-core` by a semantic-version range that resolves from the public registry.
3. THE `@corgod/kept-cli` manifest SHALL NOT carry a dependency version that resolves only within this workspace.
...
5. THE `@corgod/kept-cli` Packed_Tarball SHALL expose the `kept` binary, and that binary SHALL carry an interpreter directive on its first line.
